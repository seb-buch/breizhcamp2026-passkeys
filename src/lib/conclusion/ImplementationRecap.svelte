...
<Slide>
	...
	<SpeakerNotes>
		...
		Côté serveur en revanche, WebAuthn définit le <em>protocole</em> — les formats d'échange et les
		algorithmes de vérification — mais <em>pas d'API</em>. À vous de générer les options, vérifier
		la réponse selon les sections 7.1 et 7.2 de la spec, et stocker la passkey. Et c'est précisément
		là que les bibliothèques comme SimpleWebAuthn ou java-webauthn-server sont précieuses : elles
		encapsulent toute la complexité cryptographique et les vérifications imposées par la spec. En
		quelques appels de fonction, vous avez un flow conforme, comme on l'a vu dans KrabsVault.
	</SpeakerNotes>
</Slide>
